...
SynthDef('pad01a', {|
	...
	// snd1 = RLPF.ar(in: (Saw.ar(freq: freq) + Pulse.ar(freq: freq/2, mul: 0.1)) * 0.5, freq: freq * 1.5);
	snd1 = (Saw.ar(freq: freq) + Pulse.ar(freq: freq/2, mul: 0.04)) * 0.5;
    ...
    output = fx;
    output = output * vol; // Adjust the overall output level
...
a=Synth("pad01a", [\freq, 440, \modf, 30])
